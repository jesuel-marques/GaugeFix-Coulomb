...
inline static void gaugefixOverrelaxationLocal(Mtrx3x3* restrict U,
    ...
    /*	The above function determines update_LA which would be the naïve update to bring
        the local function to its mininum. However, using overrelaxation, which means
        using update_LA^omega instead of  update_LA, where 1<omega<2 makes the converge
        faster. update_LA^omega is calculated using the first two terms of the binomial
        expansion:
    ...
    if (power3x3Binomial(&update_LA, params.omega_OR, &update_OR)) {
        ...
        /*  If matrix could not be projected to SU3 inside power_3x3 binomial, then use
        identity as update. */
    }
...
/* Performs the gauge-fixing of a gluon-field following the overrelaxation algorithm
   with the given parameters. Sets G to identity before starting the procedure,
   overwriting anything that may have been loaded to it. */
