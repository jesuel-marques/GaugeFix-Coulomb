...
inline void setSU2Random(Mtrx2x2CK* restrict u) {
     ...
     * Parameters:
     * ==========
     * Mtrx2x2CK * u:         2x2 Cayley-Klein matrix to be set to a SU(2) random
     *                        matrix.
     *
     * Returns:
     * =======
    ...
    double abs_a, abs_b, phase_a, phase_b;
    double random_number[3];

    ranlxd(random_number, 3);

    abs_a = sqrt(random_number[0]);
    abs_b = sqrt(1.0 - abs_a * abs_a);

    phase_a = (2.0 * M_PI) * (random_number[1]);
    phase_b = (2.0 * M_PI) * (random_number[2]);

    u->m[0] = abs_a * cos(phase_a);
    u->m[1] = abs_b * sin(phase_b);
    u->m[2] = abs_b * cos(phase_b);
    u->m[3] = abs_a * sin(phase_a);
}
...
inline void convertFromCK(const Mtrx2x2CK* restrict u_ck,
    ...
    u->m[ELM_2X2(0, 0)] = u_ck->m[0] + I * u_ck->m[3];

    u->m[ELM_2X2(0, 1)] = u_ck->m[2] + I * u_ck->m[1];

    u->m[ELM_2X2(1, 0)] = -u_ck->m[2] + I * u_ck->m[1];

    u->m[ELM_2X2(1, 1)] = u_ck->m[0] - I * u_ck->m[3];
}
...
void product2x2(const Mtrx2x2CK* restrict u,
    ...
    for (MtrxIdx2 a = 1; a <= 3; a++) {
        uv->m[a] = (u->m[a]) * (v->m[0]) + (u->m[0]) * (v->m[a]) - (u_cross_v.m[a]);
    }
...
/* Projects 2x2 Cayley-Klein matrix u to the group SU(2) returning SU(2) matrix in
   u at the end. */
inline Scalar projectSU2(Mtrx2x2CK* restrict u) {
    /*
     * Calls:
     * =====
